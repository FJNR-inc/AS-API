...
class AnswersTests(APITestCase):
    ...
    def test_create_answer_without_submission(self):
        ...
        active
        """
        self.client.force_authenticate(user=self.user_without_submission)

        self.assertEqual(None, self.user_without_submission.active_submission)
        ...
        refreshed_user = get_user_model().objects.get(
            id=self.user_without_submission.id
        )
        ...
        expected_payload = {
            'submission': 'http://testserver' + reverse(
                "submission-detail",
                args=[refreshed_user.active_submission.id]
            ),
            'question': 'http://testserver' + reverse(
                "question-detail",
                args=[self.question.id]
